...
export function PlansTable({ plans, onEdit, onDelete, onViewKPIs, onViewClients }: PlansTableProps) {
  return (
        ...
        <TableHeader>
          <TableRow className="border-none">
            <TableHead className="glass-table-header">
              <div className="flex items-center gap-1 cursor-pointer">Nome <ArrowUpDown className="h-3 w-3" /></div>
            </TableHead>
            <TableHead className="glass-table-header">Download</TableHead>
            <TableHead className="glass-table-header">Upload</TableHead>
            <TableHead className="glass-table-header">
              <div className="flex items-center gap-1 cursor-pointer">Preco <ArrowUpDown className="h-3 w-3" /></div>
            </TableHead>
            <TableHead className="glass-table-header">
              <div className="flex items-center gap-1 cursor-pointer">Clientes <ArrowUpDown className="h-3 w-3" /></div>
            </TableHead>
        ...
        <TableBody>
          {plans.map((plan) => (
            <TableRow key={plan.id} className="glass-table-row border-none">
              ...
              <TableCell className="text-sm font-medium">R$ {Number(plan.monthly_price).toFixed(2)}</TableCell>
              <TableCell className="text-sm">{Number(plan.clients_count || 0).toLocaleString("pt-BR")}</TableCell>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem onClick={() => onViewKPIs(plan.id)}>
                      <BarChart3 className="h-4 w-4 mr-2" />Ver KPIs
                    </DropdownMenuItem>
