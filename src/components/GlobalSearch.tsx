...
function searchAll(query: string): SearchResult[] {
  ...
  mockClients.forEach((c) => {
    if (c.name.toLowerCase().includes(q) || c.document.includes(q) || c.phone.includes(q) || c.email.toLowerCase().includes(q)) {
      results.push({ id: c.id, label: c.name, sublabel: `${c.document} • ${c.plan} • ${c.city}`, type: "client", url: `/clients/${c.id}` });
    }
  });

  mockTickets.forEach((t) => {
    if (t.id.toLowerCase().includes(q) || t.subject.toLowerCase().includes(q) || t.client.toLowerCase().includes(q)) {
      results.push({ id: t.id, label: `${t.id} — ${t.subject}`, sublabel: `${t.client} • ${t.status}`, type: "ticket", url: "/tickets" });
    }
  });

  mockInvoices.forEach((inv) => {
    if (inv.id.toLowerCase().includes(q) || inv.client.toLowerCase().includes(q)) {
      results.push({ id: inv.id, label: `${inv.id} — R$ ${inv.amount.toFixed(2)}`, sublabel: `${inv.client} • ${inv.status}`, type: "invoice", url: "/finance" });
    }
  });

  mockEquipments.forEach((eq) => {
    if (eq.name.toLowerCase().includes(q) || eq.ip.includes(q)) {
      results.push({ id: eq.id, label: eq.name, sublabel: `${eq.ip} • ${eq.clients} clientes • ${eq.status}`, type: "equipment", url: "/network" });
    }
  });

  mockPlans.forEach((p) => {
    if (p.name.toLowerCase().includes(q)) {
      results.push({ id: p.id, label: p.name, sublabel: `${p.downloadSpeed}/${p.uploadSpeed} Mbps • R$ ${p.price.toFixed(2)}`, type: "plan", url: "/plans" });
    }
  });
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setQuery(""); setSelectedIndex(0); } }}>
        <DialogContent className="glass-card p-0 max-w-lg border-none gap-0 overflow-hidden" style={{ borderRadius: 16 }}>
          <div className="flex items-center gap-3 px-4 border-b" style={{ borderColor: "rgba(0,0,0,0.06)" }}>
            ...
            {query.length < 2 ? (
              <div className="p-6 text-center">
                ...
                <div className="flex justify-center gap-4 mt-3 text-xs text-muted-foreground">
                  <span><kbd className="px-1.5 py-0.5 rounded border bg-muted text-[10px]">↑↓</kbd> navegar</span>
                  <span><kbd className="px-1.5 py-0.5 rounded border bg-muted text-[10px]">↵</kbd> selecionar</span>
                  <span><kbd className="px-1.5 py-0.5 rounded border bg-muted text-[10px]">esc</kbd> fechar</span>
                </div>
              </div>
            ) : results.length === 0 ? (
              <div className="p-6 text-center">
                ...
                {results.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-muted-foreground font-medium px-2 py-0.5 rounded-full" style={{ background: "rgba(0,0,0,0.04)" }}>
                        {typeLabels[r.type]}
                      </span>
                    </button>
                  );
